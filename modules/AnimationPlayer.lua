	...
	self._heartbeatConnection = RunService.RenderStepped:Connect(function(dt)
		...
		for i = #self.activeAnimations, 1, -1 do
			...
			if anim.playing then
				...
				if not anim.looped and anim.time >= 0.999 then
					anim.time = 0.999
					if not anim.remove and anim.targetWeight ~= 0 then
						self:stopAnimation(anim.name, 0.1)
					end
				end
			end

			if anim.fadeSpeed and anim.fadeSpeed > 0 then
				...
				-- If no fadeSpeed, jump directly to target weight
				anim.weight = anim.targetWeight

				if anim.weight == 0 then
					anim.remove = true
				end
			end
...
function animationPlayer:checkEvents(animationName, lastTime, currentTime, looped)
	...
	for _, keyFrame in animation.keyFrames do
		local eventFiredKey = tostring(keyFrame.time)

		-- Check if event should fire in current time window
		local inRange = false
		if looped and currentTime < lastTime then
			inRange = keyFrame.time >= lastTime - EPSILON or keyFrame.time <= currentTime + EPSILON
		else
			inRange = keyFrame.time >= lastTime - EPSILON and keyFrame.time <= currentTime + EPSILON
		end

		if inRange and keyFrame.event then
			local anim = self:getAnimation(animationName)
			if anim and not anim.firedEvents[eventFiredKey] then
				anim.firedEvents[eventFiredKey] = true
